...
st.write("Enter the top 7 house features using sliders and click **Predict Price**.")

# -------------------------
# 2️⃣ Load median values for other features
# -------------------------
@st.cache_data  # ← caches the CSV so it doesn't reload every time
def load_data():
    df = pd.read_csv(
...
with st.form("house_form"):
    ...
    with col2:
        ...
        sqft_living15 = st.slider("Neighbour Living Area (sqft)", min_value=300, max_value=8000, value=1700)
...
if submit:
    ...
    with st.spinner("⏳ Waking up server & predicting... (may take ~30s on first request)"):
        try:
            ...
            st.error("⏱️ Server is taking too long to respond. Please try again in a moment.")
...
# -------------------------
st.markdown("---")
st.write("Built by **P. Sumanth Pola** 🚀 | FastAPI + XGBoost + Streamlit")